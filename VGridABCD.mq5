...
// Chế độ vào lệnh chờ (Chờ ảo / broker).
...
input int    VGridScaleStartLevelBuyA = 1;      // Bậc lưới bắt đầu gấp thếp (>=1); bậc < X giữ lot L1
...
input double VGridTradingStopStepPipsBuyA = 0.0;    // Trading Stop 3a: cứ đi thêm X pip thuận lợi thì dời SL thêm X p
...
input int    VGridScaleStartLevelBuyB = 1;      // Bậc lưới bắt đầu gấp thếp (>=1); bậc < X giữ lot L1
...
input double VGridTradingStopStepPipsBuyB = 0.0;    // Trading Stop 3b: cứ đi thêm X pip thuận lợi thì dời SL thêm X p
...
input int    VGridScaleStartLevelSellC = 1;     // Bậc lưới bắt đầu gấp thếp (>=1); bậc < X giữ lot L1
...
input double VGridTradingStopStepPipsSellC = 0.0;    // Trading Stop 3c: cứ đi thêm X pip thuận lợi thì dời SL thêm X p
...
input int    VGridScaleStartLevelSellD = 1;     // Bậc lưới bắt đầu gấp thếp (>=1); bậc < X giữ lot L1
...
input double VGridTradingStopStepPipsSellD = 0.0;    // Trading Stop 3d: cứ đi thêm X pip thuận lợi thì dời SL thêm X p
...
input bool   EnableSessionFloatLossCompoundTriggerAdjust = false; // Bật: float phiên <= -X -> ngưỡng gốc gồng lãi tổng = ngưỡng mới (vẫn + carry)
...
input bool   EnableGridCommonStopLoss = false;  // Bật: SL chung Buy dưới gốc / Sell trên gốc (X pip); lỗ khi chạm
...
input double CompoundTotalProfitTriggerUSD = 5.0; // Ngưỡng gốc (USD); ngưỡng thực = gốc (+ carry nếu bật carry)
input ENUM_COMPOUND_CLEAR_PENDING_MODE CompoundClearPendingOnCommonSl = COMPOUND_KEEP_PENDING; // Khi đặt SL chung gồng: chờ ảo/broker
input bool   EnableCompoundSlPauseUntilNextServerDay = false; // Bật: gồng lãi tổng chạm SL chung -> tạm dừng EA tới ngày server kế tiếp
input ENUM_COMPOUND_POINT_A_MODE CompoundPointAMode = COMPOUND_POINT_A_MODE_1; // Chế độ xác định điểm A (1 hoặc 2)
...
input double StartupRsiCrossUpLevel = 60.0;              // X1 cắt lên (RSI shift1 > X1 và shift2 <= X1); 0 = không dùng
input int    StartupRsiPreCrossUpBarsBelowX1 = 0;        // Trước cắt lên X1: X nến đóng liên tiếp RSI < X1; 0 = bỏ
input double StartupRsiCrossDownLevel = 40.0;            // X2 cắt xuống (RSI shift1 < X2 và shift2 >= X2); 0 = không dùng
...
input ENUM_EMA_DIRECTION_MODE EmaDirectionMode = EMA_DIRECTION_CLOSE; // Chế độ lọc EMA
...
input group "6c - Nến — lọc chiều lưới —"
input bool   EnableCandleDirectionFilter = true;  // Bật: nến đóng xanh=chỉ Buy; đỏ=chỉ Sell (đến reset EA)
input ENUM_TIMEFRAMES CandleDirectionTimeframe = PERIOD_D1; // Khung nến; PERIOD_CURRENT = khung chart
input int    CandleDirectionConsecutiveBars = 1;  // Số nến đóng liên tiếp cùng chiều (>=1); 2 = 2 nến xanh/đỏ
...
double g_compoundFrozenRefPx = 0.0;           // Điểm A (khóa lần đầu khi ≥ ngưỡng; xóa khi < ngưỡng)
...
bool     g_compoundCommonSlHitPendingReset = false; // (legacy, không dùng)
...
long     g_mpDailyTrackDateKey = 0;            // ngày server đang theo dõi DD (TEV)
double   g_mpDailyPeakTev = 0.0;               // TEV cao nhất trong ngày server hiện tại
double   g_mpDailyMinTev = 0.0;                // TEV thấp nhất trong ngày server hiện tại
double   g_mpDailyMaxDrawdownUsd = 0.0;        // DD lớn nhất trong ngày (peak−min TEV)
...
int      g_candleDirectionLock = 0;            // 0=chưa khóa; +1=nến xanh chỉ Buy; −1=nến đỏ chỉ Sell (khóa lúc đặt gốc)
datetime g_candleDirectionWaitLoggedBar = 0;   // nến shift1 đã log "chờ nến rõ chiều" (Doji)
...
void CandleDirectionClearLock();
bool CandleDirectionReadClosedBarSignal(int &signalOut);
bool CandleDirectionTrySetLockFromClosedBar();
void CandleDirectionSnapshotLockAtSessionStart();
void CandleDirectionPollLockIfNeeded();
void CandleDirectionLogWaitIfNeeded();
bool CandleDirectionAllowsBasePlacement();
bool CandleDirectionAllowsLeg(const ENUM_VGRID_LEG leg);
void CandleDirectionPurgeBlockedSidePendings();
bool GridDirectionAllowsLeg(const ENUM_VGRID_LEG leg);
void GridDirectionPurgeBlockedSidePendings();
...
void MonthlyProfitPanelUpdateDailyDrawdownTracking();
...
//+------------------------------------------------------------------+
//| Có điểm A → gỡ TP (snapshot lần đầu); mất điểm A → khôi phục TP.  |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Lần đầu ≥ ngưỡng: tìm và khóa A. Vẫn ≥ ngưỡng: giữ A. < ngưỡng: xóa. |
//+------------------------------------------------------------------+
void CompoundRefreshTrackingReference(const double compoundTriggerProgressUsd)
{
   ...
   // Vẫn trên ngưỡng: giữ nguyên điểm A lần đầu (dù tiến độ/ngưỡng tiếp tục tăng).
   ...
   Print("VGridABCD: Gồng lãi — ngưỡng ", DoubleToString(thresholdUsd, 2), " USD OK (lần đầu)",
         ...
         " | chờ giá cách A ≥1 bậc lưới → SL tại A.");
}

//+------------------------------------------------------------------+
//| Bước 2: giá cách điểm A ≥1 bậc lưới → SL tại A (+ xóa chờ nếu bật). |
//| Buy: Bid ≥ mức lưới trên A; Sell: Ask ≤ mức lưới dưới A.         |
//| TP: chỉ gỡ ở giai đoạn 1 (có điểm A) — không gỡ lại tại đây.     |
//+------------------------------------------------------------------+
void ProcessCompoundWaitingFirstGridStep()
{
   ...
   Print("VGridABCD: Gồng lãi — giá cách A ≥1 bậc lưới: SL chung tại A=",
         ...
         (CompoundClearPendingOnCommonSl == COMPOUND_CLEAR_PENDING
            ? " | xóa chờ ảo/broker | không bổ sung chờ mới"
            : " | giữ nguyên chờ ảo/broker | không bổ sung chờ mới"));
}
...
//+------------------------------------------------------------------+
//| SL chung gồng lãi bị chạm → luôn reset toàn bộ EA + carry.        |
//+------------------------------------------------------------------+
void CompoundTryResetAfterCommonSlHit(const string reason)
{
   if(!g_compoundTotalProfitActive)
   ...
   Print("VGridABCD: Gồng lãi — ", reason,
         ...
         " → đóng hết lệnh, xóa gốc, reset EA.");
   CompoundResetAfterCommonSlHit();
}

//+------------------------------------------------------------------+
//| Chạm SL chung → đóng hết lệnh, xóa gốc/lưới, reset EA ngay lập tức. |
//+------------------------------------------------------------------+
void CompoundResetAfterCommonSlHit()
{
   g_compoundCommonSlHitPendingReset = false;
...
void ProcessCompoundTotalProfitTrailing()
{
   ...
   {
      CompoundTryResetAfterCommonSlHit("giá chạm SL chung");
      return;
   }
...
//+------------------------------------------------------------------+
//| Nến lọc chiều: N nến đóng liên tiếp xanh → Buy; đỏ → Sell.       |
//| Khóa chiều lúc đặt gốc; giữ đến reset EA. Phiên Buy/Sell dở      |
//| vẫn tiếp tục chiều đang chạy.                                    |
//+------------------------------------------------------------------+
ENUM_TIMEFRAMES CandleDirectionResolvedTimeframe()
{
   ENUM_TIMEFRAMES tf = CandleDirectionTimeframe;
   if(tf == PERIOD_CURRENT)
      tf = (ENUM_TIMEFRAMES)_Period;
   return tf;
}

int CandleDirectionResolvedConsecutiveBars()
{
   int n = CandleDirectionConsecutiveBars;
   if(n < 1)
      n = 1;
   return n;
}
...
void CandleDirectionClearLock()
{
   g_candleDirectionLock = 0;
   g_candleDirectionWaitLoggedBar = 0;
}
...
bool CandleDirectionReadClosedBarSignal(int &signalOut)
{
   ...
   const ENUM_TIMEFRAMES tf = CandleDirectionResolvedTimeframe();
   const int needBars = CandleDirectionResolvedConsecutiveBars();
   if(Bars(_Symbol, tf) < needBars + 1)
      return false;
   ...
   ArraySetAsSeries(closeBuf, true);
   if(CopyOpen(_Symbol, tf, 1, needBars, openBuf) < needBars)
      return false;
   if(CopyClose(_Symbol, tf, 1, needBars, closeBuf) < needBars)
      return false;
   ...
   {
      if(!MathIsValidNumber(openBuf[i]) || !MathIsValidNumber(closeBuf[i]))
         return false;
      if(closeBuf[i] <= openBuf[i])
         allGreen = false;
      if(closeBuf[i] >= openBuf[i])
         allRed = false;
   }
...
bool CandleDirectionSessionHasBuyActivity()
{
   for(int i = 0; i < ArraySize(g_virtualPending); i++)
   {
      if(IsVirtualGridLegBuyEntryLeg(g_virtualPending[i].leg))
         return true;
   }
   for(int i = 0; i < PositionsTotal(); i++)
   {
      ...
      if(sessionStartTime > 0 && (datetime)PositionGetInteger(POSITION_TIME) < sessionStartTime)
         continue;
      ENUM_VGRID_LEG leg = VGRID_LEG_BUY_ABOVE;
      if(TryParseLegFromOrderComment(PositionGetString(POSITION_COMMENT), leg)
         && IsVirtualGridLegBuyEntryLeg(leg))
         return true;
   }
   return false;
}

bool CandleDirectionSessionHasSellActivity()
{
   for(int i = 0; i < ArraySize(g_virtualPending); i++)
   {
      if(IsVirtualGridLegSellEntryLeg(g_virtualPending[i].leg))
         return true;
   }
   for(int i = 0; i < PositionsTotal(); i++)
   {
      ...
      if(sessionStartTime > 0 && (datetime)PositionGetInteger(POSITION_TIME) < sessionStartTime)
         continue;
      ENUM_VGRID_LEG leg = VGRID_LEG_SELL_ABOVE;
      if(TryParseLegFromOrderComment(PositionGetString(POSITION_COMMENT), leg)
         && IsVirtualGridLegSellEntryLeg(leg))
         return true;
   }
   return false;
}
...
bool CandleDirectionTrySetLockFromClosedBar()
{
   if(!EnableCandleDirectionFilter)
      return true;
   if(g_candleDirectionLock != 0)
      return true;

   int signal = 0;
   if(!CandleDirectionReadClosedBarSignal(signal) || signal == 0)
      return false;

   g_candleDirectionLock = signal;
   ...
   ArraySetAsSeries(openBuf, true);
   ArraySetAsSeries(closeBuf, true);
   CopyOpen(_Symbol, tf, 1, needBars, openBuf);
   CopyClose(_Symbol, tf, 1, needBars, closeBuf);
   ...
   for(int i = 0; i < needBars; i++)
   {
      if(i > 0)
         barsDetail += " | ";
      barsDetail += "[" + IntegerToString(i + 1) + "] O=" + DoubleToString(openBuf[i], dgt)
                 + " C=" + DoubleToString(closeBuf[i], dgt);
   }

   Print("VGridABCD: Nến lọc chiều — khóa phiên ",
         (g_candleDirectionLock > 0 ? "chỉ Buy (nến xanh)" : "chỉ Sell (nến đỏ)"),
         " | ", needBars, " nến liên tiếp | ", EnumToString(tf),
         " | ", barsDetail,
         " (giữ đến khi EA reset)");
   return true;
}
...
void CandleDirectionSnapshotLockAtSessionStart()
{
   ...
   if(!EnableCandleDirectionFilter)
      return;
   CandleDirectionTrySetLockFromClosedBar();
}

void CandleDirectionPollLockIfNeeded()
{
   if(!EnableCandleDirectionFilter || g_candleDirectionLock != 0)
      return;
   CandleDirectionTrySetLockFromClosedBar();
}
...
void CandleDirectionLogWaitIfNeeded()
{
   if(!EnableCandleDirectionFilter || basePrice > 0.0)
      return;
   if(!StartupRsiAllowsBasePlacement())
      return;
   ...
   const ENUM_TIMEFRAMES tf = CandleDirectionResolvedTimeframe();
   const datetime bar1 = iTime(_Symbol, tf, 1);
   if(bar1 <= 0 || bar1 == g_candleDirectionWaitLoggedBar)
      return;

   int signal = 0;
   if(!CandleDirectionReadClosedBarSignal(signal))
      return;
   if(signal != 0)
      return;

   g_candleDirectionWaitLoggedBar = bar1;
   const int needBars = CandleDirectionResolvedConsecutiveBars();
   Print("VGridABCD: Nến lọc chiều — chờ đặt gốc (chưa đủ ",
         needBars, " nến đóng liên tiếp cùng chiều) | ",
         EnumToString(tf));
}
...
bool CandleDirectionAllowsBuyEntries()
{
   if(!EnableCandleDirectionFilter)
      return true;
   if(g_candleDirectionLock > 0)
      return true;
   if(CandleDirectionSessionHasBuyActivity())
      return true;
   return false;
}

bool CandleDirectionAllowsSellEntries()
{
   if(!EnableCandleDirectionFilter)
      return true;
   if(g_candleDirectionLock < 0)
      return true;
   if(CandleDirectionSessionHasSellActivity())
      return true;
   return false;
}
...
{
   if(IsVirtualGridLegBuyEntryLeg(leg))
      return CandleDirectionAllowsBuyEntries();
   if(IsVirtualGridLegSellEntryLeg(leg))
      return CandleDirectionAllowsSellEntries();
   return true;
}
...
void CandleDirectionPurgeBlockedSidePendings()
{
   if(!EnableCandleDirectionFilter)
      return;
   if(g_candleDirectionLock == 0
      && !CandleDirectionSessionHasBuyActivity()
      && !CandleDirectionSessionHasSellActivity())
      return;

   const bool allowBuy = CandleDirectionAllowsBuyEntries();
   const bool allowSell = CandleDirectionAllowsSellEntries();
...
bool GridDirectionAllowsLeg(const ENUM_VGRID_LEG leg)
{
   return (EmaDirectionAllowsLeg(leg) && CandleDirectionAllowsLeg(leg));
}

void GridDirectionPurgeBlockedSidePendings()
{
   EmaDirectionPurgeBlockedSidePendings();
   CandleDirectionPurgeBlockedSidePendings();
}
...
bool TryPlaceBaseAfterStartupFilters()
{
   ...
   if(!EmaDirectionAllowsBasePlacement())
      return false;
   if(!CandleDirectionAllowsBasePlacement())
      return false;

   basePrice = GridBasePriceAtPlacement();
   InitializeGridLevels();
   GridDirectionPurgeBlockedSidePendings();
   if(EnableResetNotification)
...
void ProcessVirtualPendingExecutions()
{
   ...
   for(int i = ArraySize(g_virtualPending) - 1; i >= 0; i--)
   {
      ...
      if(!GridDirectionAllowsLeg(e.leg))
      {
         VirtualPendingRemoveAt(i);
         continue;
      }
...
void MonthlyProfitPanelOnInitState()
{
   ...
   MonthlyProfitPanelUpdateDailyDrawdownTracking();
}

//+------------------------------------------------------------------+
//| Theo dõi DD lớn nhất trong ngày server (TEV peak − min).          |
//+------------------------------------------------------------------+
void MonthlyProfitPanelUpdateDailyDrawdownTracking()
{
   const long dk = ServerDateKey(TimeCurrent());
   const double tev = GetTradingEquityViewUSD();
   if(g_mpDailyTrackDateKey != dk)
   {
      g_mpDailyTrackDateKey = dk;
      g_mpDailyPeakTev = tev;
      g_mpDailyMinTev = tev;
      g_mpDailyMaxDrawdownUsd = 0.0;
      return;
   }
   if(tev > g_mpDailyPeakTev)
      g_mpDailyPeakTev = tev;
   if(tev < g_mpDailyMinTev)
      g_mpDailyMinTev = tev;
   const double dd = g_mpDailyPeakTev - g_mpDailyMinTev;
   if(dd > g_mpDailyMaxDrawdownUsd)
      g_mpDailyMaxDrawdownUsd = dd;
}

struct MpHistDealRec
{
   datetime timeUtc;
   int      dayIdx;
   double   pnlUsd;
...
void MpSortHistDealsByTime(MpHistDealRec &arr[])
{
   const int n = ArraySize(arr);
   for(int i = 1; i < n; i++)
   {
      MpHistDealRec key = arr[i];
      int j = i - 1;
      while(j >= 0 && arr[j].timeUtc > key.timeUtc)
      {
         arr[j + 1] = arr[j];
         j--;
      }
...
void MpComputeDailyMaxDrawdownFromDeals(MpHistDealRec &deals[], double &dayMaxDD[])
{
   ArrayInitialize(dayMaxDD, 0.0);
   if(ArraySize(deals) < 1)
      return;

   MpSortHistDealsByTime(deals);

   double dayCumPnL[32];
   double dayRunPeak[32];
   ArrayInitialize(dayCumPnL, 0.0);
   ArrayInitialize(dayRunPeak, 0.0);

   for(int i = 0; i < ArraySize(deals); i++)
   {
      const int di = deals[i].dayIdx;
      if(di < 1 || di > 31)
         continue;
      dayCumPnL[di] += deals[i].pnlUsd;
      if(dayCumPnL[di] > dayRunPeak[di])
         dayRunPeak[di] = dayCumPnL[di];
      const double dd = dayRunPeak[di] - dayCumPnL[di];
      if(dd > dayMaxDD[di])
         dayMaxDD[di] = dd;
   }
}

string MpFormatDailyDrawdownText(const double ddUsd)
{
   if(ddUsd <= 1e-8)
      return "";
   return "DD -" + DoubleToString(ddUsd, 2);
}
...
void MonthlyProfitPanelRedrawIfNeeded(const bool force)
{
   ...
   static double dayMaxDD[32];
   ArrayInitialize(dayPnl, 0.0);
   ArrayInitialize(dayDeals, 0);
   ArrayInitialize(dayMaxDD, 0.0);

   MpHistDealRec histDeals[];
   ArrayResize(histDeals, 0);
   ...
   if(HistorySelect(tFrom, tTo))
   {
      ...
      for(int i = 0; i < nd; i++)
      {
         ...
         const int hn = ArraySize(histDeals);
         ArrayResize(histDeals, hn + 1);
         histDeals[hn].timeUtc = dt;
         histDeals[hn].dayIdx = di;
         histDeals[hn].pnlUsd = fullPnL;
      }
   }

   MpComputeDailyMaxDrawdownFromDeals(histDeals, dayMaxDD);
   ...
   MonthlyProfitPanelUpdateDailyDrawdownTracking();
   if(isViewingCurrentMonth && srvNow.day >= 1 && srvNow.day <= 31)
   {
      if(g_mpDailyMaxDrawdownUsd > dayMaxDD[srvNow.day])
         dayMaxDD[srvNow.day] = g_mpDailyMaxDrawdownUsd;
   }
   ...
   for(int row = 0; row < 6; row++)
   {
      for(int col = 0; col < 7; col++)
      {
         ...
         if(dayDeals[curDay] > 0)
         {
            ...
            lineDD = MpFormatDailyDrawdownText(dayMaxDD[curDay]);
         }
         else if(isToday)
         {
         ...
         if(lineDD != "")
            MpLabelCreate(MpPanelObjPrefix() + "dd" + IntegerToString(curDay), cellX + 4, cellY + 44, lineDD, f0 - 2, C_RED, false, crn);
...
int OnInit()
{
   ...
   if(g_runtimeSessionActive)
   {
      if(!TryPlaceBaseAfterStartupFilters())
      {
         ...
         CandleDirectionLogWaitIfNeeded();
      }
   }
   else
   {
   ...
   if(EnableCandleDirectionFilter)
      Print("Nến lọc chiều: BẬT | ", EnumToString(CandleDirectionResolvedTimeframe()),
            " | ", CandleDirectionResolvedConsecutiveBars(), " nến liên tiếp xanh=Buy; đỏ=Sell; khóa lúc đặt gốc đến reset",
            (g_candleDirectionLock > 0 ? " | phiên Buy" : (g_candleDirectionLock < 0 ? " | phiên Sell" : " | chờ nến rõ chiều")));
   ...
   if(basePrice <= 0.0)
   {
      ...
      Print("VGridABCD: đặt gốc — base=", DoubleToString(basePrice, dgt),
            ...
            (EmaDirectionUsesHighLowMode()
               ...
               : ""),
            (EnableCandleDirectionFilter
               ? (g_candleDirectionLock > 0 ? " (Nến xanh Buy)" : " (Nến đỏ Sell)")
               : ""));
      if(EnableResetNotification)
                        ...
                        const MqlTradeResult& result)
{
   ...
   {
      CompoundTryResetAfterCommonSlHit("broker khớp SL chung");
...
//+------------------------------------------------------------------+
//| Bậc lưới |X| bắt đầu gấp thếp (bậc < X giữ lot L1).                |
//+------------------------------------------------------------------+
int VirtualGridResolvedScaleStartLevel(const ENUM_VGRID_LEG leg)
{
   int startLvl = 1;
   switch(leg)
   {
      case VGRID_LEG_BUY_ABOVE:
      case VGRID_LEG_BUY_BELOW: startLvl = VGridScaleStartLevelBuyA; break;
      case VGRID_LEG_BUY_ABOVE_E:
      case VGRID_LEG_BUY_BELOW_H: startLvl = VGridScaleStartLevelBuyB; break;
      case VGRID_LEG_SELL_ABOVE:
      case VGRID_LEG_SELL_BELOW: startLvl = VGridScaleStartLevelSellC; break;
      case VGRID_LEG_SELL_ABOVE_G:
      case VGRID_LEG_SELL_BELOW_F: startLvl = VGridScaleStartLevelSellD; break;
   }
   if(startLvl < 1)
      startLvl = 1;
   if(startLvl > MaxGridLevels)
      startLvl = MaxGridLevels;
   return startLvl;
}
...
//+------------------------------------------------------------------+
//| LOT theo chân: L1; gấp thếp từ bậc X (bậc < X giữ L1).            |
//+------------------------------------------------------------------+
double GetLotForVirtualGridLeg(const ENUM_VGRID_LEG leg, const int absLevelRaw)
{
   ...
   const int startLvl = VirtualGridResolvedScaleStartLevel(leg);

   const double baseLot = GetBaseLotForVirtualGridLeg(leg);
   const ENUM_LOT_SCALE scale = GetLotScaleForVirtualGridLeg(leg);
   double lot = baseLot;
   if(absLevel < startLvl || scale == LOT_FIXED)
      lot = baseLot;
   else if(scale == LOT_ARITHMETIC)
      lot = baseLot + (double)(absLevel - startLvl) * GetLotAddForVirtualGridLeg(leg);
   else if(scale == LOT_GEOMETRIC)
      lot = baseLot * MathPow(GetLotMultForVirtualGridLeg(leg), (double)(absLevel - startLvl));
...
void InitializeGridLevels()
{
   ...
   CandleDirectionSnapshotLockAtSessionStart();
   CompoundFloatThrHudUpdate(true);
}
...
void ManageGridOrders()
{
   ...
   CandleDirectionPollLockIfNeeded();
   if(EnableCandleDirectionFilter && g_candleDirectionLock == 0
      && !CandleDirectionSessionHasBuyActivity()
      && !CandleDirectionSessionHasSellActivity())
      return;
